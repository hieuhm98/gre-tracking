...
**JavaScript (JS)** is a programming language that runs directly in the browser, turning a web page from static content into a *dynamic* and *interactive* application. JS can also run on the server via **Node.js**.

**The role of JS in the web:**
- Handling user interactions (click, scroll, hover)
- Validating forms before sending them to the server
- Updating the interface without reloading the page
- Calling APIs to fetch/send data to and from the server
...
**DOM** (Document Object Model) is how JS interacts with the HTML structure:
...
### Async/Await (the most modern approach)
...
## 12. Fetch API — Making HTTP Requests
...
## 13. LocalStorage and SessionStorage
...
**Concepts a BA needs to understand:**

**1. Client-side vs Server-side validation:**
- JS validation on the client = good UX, but NOT enough for security
- The BE MUST validate again — the BA needs to specify which rules are needed at both layers

**2. UI state:**
- **Loading**: an API call is in progress
- **Success**: data is available
- **Error**: the API call failed
- **Empty**: succeeded but there is no data
→ The BA must describe all 4 states in the AC

**3. Async UX:**
- An API call takes 0.5–3 seconds → a loading indicator is needed
- Timeout → an error message and retry are needed
- The BA needs to specify: "if the API does not respond within 10s, show an error and a Retry button"

**4. Event-driven thinking:**
- Every user action is an event
- The BA should list them out: "When the user clicks X → Y happens → Z is displayed"

**5. SPA (Single Page Application):**
- The page does not reload when switching screens
- The URL changes but the page is not reloaded
- Affects the browser back button, deep linking, and SEO
